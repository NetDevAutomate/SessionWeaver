...
The upstream ``agent-session-tools`` migration owner retains ``PRAGMA user_version``.
This sidecar records and verifies its own schema identity so the same definitions can
be lifted unchanged into the upstream migration in Phase B3.
"""
...
def _create_objects(conn: sqlite3.Connection, objects: tuple[_SchemaObject, ...]) -> None:
    for item in objects:
        conn.execute(item.sql)


def _verify_clock(conn: sqlite3.Connection) -> None:
    expected = conn.execute("SELECT instance FROM context_access_state WHERE id=1").fetchone()
    actual = conn.execute(
...
def _ensure_schema(conn: sqlite3.Connection) -> None:
    ...
    with _atomic(conn):
        ...
        instance = conn.execute("SELECT instance FROM context_access_state WHERE id=1").fetchone()
        if instance is None:
            raise RuntimeError("Pinned context instance identity is missing")
        conn.execute("INSERT INTO context_concept_clock VALUES (1,?,0,0)", (instance[0],))
        _create_objects(conn, _METADATA_OBJECTS)
        conn.execute(
            "INSERT INTO context_concept_schema VALUES (1,?,?)",
            (SCHEMA_VERSION, SCHEMA_FINGERPRINT),
