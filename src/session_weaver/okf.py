...
_FIELDS: Final = frozenset(
    {"type", "title", "description", "tags", "sources", "verified", "confidence", "actor"}
)
...
class ImportReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        payload: dict[str, Any] = {name: cast(int, getattr(self, name)) for name in _COUNTER_NAMES}
...
def _load_frontmatter(value: str) -> Any:
    ...
    return yaml.load(value, Loader=_StrictSafeLoader)
...
def _schema_record(
    ...
    *,
    ...
    elif not math.isfinite(float(raw_confidence)) or not 0.5 <= float(raw_confidence) <= 1.0:
    ...
    if not isinstance(raw_verified, dict):
        ...
        verifier, verifier_issues = _text(raw_verified.get("by"), field="/verified/by", maximum=128)
