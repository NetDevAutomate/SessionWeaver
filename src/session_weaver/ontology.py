...
_RE_PATH = re.compile(r"(?<![\w.])(/Users/[a-z]+/[\w./~-]{8,140}\.[A-Za-z0-9]{1,8})(?![\w/])")
_RE_CMD = re.compile(
    r"(?:^\$ |^```(?:bash|zsh|sh)\n)([a-z][\w.-]+(?: [^\n`]{2,120})?)",
    re.M,
)
...
def canonical_messages(
    ...
    for message_id, session_id, role, content, timestamp, seq in conn.execute(query, parameters):
        if content is None:
            continue
        text = content.strip()
        if not text:
    ...
    rows.sort(key=lambda message: (message.session_id, *_canonical_message_key(message)))
...
def _extract_structural(
    ...
    return tuple(sorted(rows.values(), key=lambda row: (row.session_id, row.type, row.key)))
...
def _build_graph(
    ...
    for session in sessions:
        ...
        if project is None:
            raise OntologyValidationError(f"session has no structural project: {session.id}")
...
def _drop_tables(conn: sqlite3.Connection, *, staging: bool) -> None:
    names = _LIVE_TO_STAGING if staging else {table: table for table in ONTOLOGY_TABLES}
    for table in (
        "ontology_relation",
        "ontology_individual",
        "ontology_property",
...
def _create_staging_tables(conn: sqlite3.Connection) -> None:
    statements = (
        """
        CREATE TABLE __ontology_class_next(
            name TEXT PRIMARY KEY CHECK(length(name) > 0),
            parent TEXT REFERENCES __ontology_class_next(name)
                DEFERRABLE INITIALLY DEFERRED,
            description TEXT NOT NULL CHECK(length(description) > 0)
        )
        """,
        """
        CREATE TABLE __ontology_property_next(
            name TEXT PRIMARY KEY CHECK(length(name) > 0),
            domain TEXT NOT NULL REFERENCES __ontology_class_next(name),
            range TEXT NOT NULL REFERENCES __ontology_class_next(name),
            description TEXT NOT NULL CHECK(length(description) > 0)
        )
        """,
        """
        CREATE TABLE __ontology_structural_next(
        ...
        """,
        """
        CREATE TABLE __ontology_individual_next(
            id TEXT PRIMARY KEY CHECK(length(id) > 0),
            class TEXT NOT NULL REFERENCES __ontology_class_next(name),
            label TEXT NOT NULL CHECK(length(label) <= 200),
            attrs TEXT NOT NULL CHECK(json_valid(attrs))
        )
        """,
        """
        CREATE TABLE __ontology_relation_next(
            subject TEXT NOT NULL REFERENCES __ontology_individual_next(id),
            predicate TEXT NOT NULL REFERENCES __ontology_property_next(name),
            object TEXT NOT NULL REFERENCES __ontology_individual_next(id),
            PRIMARY KEY(subject, predicate, object)
        ) WITHOUT ROWID
        """,
        """
        CREATE TABLE __ontology_build_state_next(
...
def _validate_staging(conn: sqlite3.Connection, source_session_count: int) -> None:
    ...
    if foreign_key_errors:
        raise OntologyValidationError(f"staging foreign-key violations: {len(foreign_key_errors)}")
    ...
    if (
        ...
        raise OntologyValidationError("staging does not contain one project per session")
    ...
    if versions and versions != {EXTRACTION_VERSION}:
        raise OntologyValidationError(f"unexpected staging extraction versions: {versions!r}")
    domain_range = _domain_range_violations(conn, suffix="_next")
    if domain_range:
        raise OntologyValidationError(f"staging domain/range violations: {domain_range}")
...
def _read_build_state(
    ...
    versions = {
        row[0]
        for row in conn.execute("SELECT DISTINCT extraction_version FROM ontology_structural")
    }
    if versions and versions != {EXTRACTION_VERSION}:
        return (None, "extraction-version-mismatch")
    return (
...
def _incremental_candidates(
    ...
    project_sessions = {
        row[0]
        for row in conn.execute("SELECT session_id FROM ontology_structural WHERE type = 'project'")
    }
...
def _copied_structural(
    ...
    for raw_row in rows:
        ...
        if row.session_id not in reusable_ids:
            continue
        expected = _make_structural(row.session_id, row.type, row.key, row.value, row.ts)
...
def _plan_extraction(
    ...
    *,
    ...
    candidates, fallback_reason = _incremental_candidates(conn, sessions, message_counts, state)
    ...
    return _ExtractionPlan(
        ...
        structural=tuple(
            sorted(combined.values(), key=lambda row: (row.session_id, row.type, row.key))
        ),
    )
...
def rebuild_ontology(
    ...
    *,
    ...
    if conn.in_transaction:
        raise OntologyError("rebuild_ontology requires a connection outside a transaction")
    ...
    try:
        ...
        if conn.execute("PRAGMA foreign_key_check").fetchall():
            raise OntologyValidationError("live graph has foreign-key violations after swap")
        conn.commit()
...
def _check_ontology_schema(conn: sqlite3.Connection) -> _OntologySchemaCheck:
    ...
    for table in sorted(ONTOLOGY_TABLES & present_tables):
        ...
        if actual_columns != expected_columns:
            schema_errors.append(f"{table} columns {actual_columns!r} != {expected_columns!r}")
...
def _check_ontology_integrity(
    ...
    if orphan_structural_rows:
        diagnostics.append(f"structural rows reference absent sessions: {orphan_structural_rows}")
