...
def _capture_snapshot(context: AgentContext) -> _Snapshot:
    ...
    logical_hash = hashlib.sha256(_canonical_json(logical_rows).encode("utf-8")).hexdigest()
    return _Snapshot(
...
def _render(concept: _ProjectedConcept) -> bytes:
    citation_binding = "machine-confirmed" if concept.binding_state == "bound" else "absent"
    provenance = (
...
def _render_all(snapshot: _Snapshot) -> dict[str, _Rendered]:
    ...
    for concept in snapshot.concepts:
        ...
        rendered[name] = _Rendered(concept_id=concept.concept_id, payload=_render(concept))
...
def _recheck_output(output: _OutputDirectory) -> None:
    metadata = os.stat(output.name, dir_fd=output.parent_descriptor, follow_symlinks=False)
    if not stat.S_ISDIR(metadata.st_mode) or (metadata.st_dev, metadata.st_ino) != output.identity:
...
def _write_atomic(
    ...
    descriptor = os.open(temporary, _FILE_CREATE_FLAGS, 0o600, dir_fd=directory_descriptor)
...
def _generated_name(name: str) -> bool:
    ...
    return bool(slug) and all(
        part and all(character.isalnum() for character in part) for part in slug.split("-")
    )
...
def _preflight(
    ...
    for name, entry in previous.items():
        ...
        try:
            ...
            conflicts += 1
            continue
        if payload is not None and not _owned_bytes(payload, entry["concept_id"], entry["sha256"]):
            conflicts += 1
            continue
    ...
    for name, desired in rendered.items():
        ...
        try:
            ...
            conflicts += 1
            continue
        if payload == desired.payload and _owned_bytes(payload, desired.concept_id, desired.sha256):
            unchanged.append(name)
        else:
            conflicts += 1
        ...
        for mutation in reversed(self.mutations):
            try:
                ...
                current_matches_written = (
                    ...
                    and final_exists
                    and _matching_identity(self.descriptor, mutation.name, mutation.written)
                )
                if mutation.written is not None and final_exists and not current_matches_written:
                    conflicts += 1
...
def project_concepts(db: Path, out: Path, *, project: str | None = None) -> ProjectionReport:
    ...
    try:
        with read_boundary() as boundary:
            ...
            if preflight.conflicts:
                return _report(snapshot, conflicts=preflight.conflicts, status="conflict")
