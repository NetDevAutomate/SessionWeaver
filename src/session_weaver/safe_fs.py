...
_FILE_READ_FLAGS: Final = os.O_RDONLY | getattr(os, "O_NOFOLLOW", 0) | getattr(os, "O_CLOEXEC", 0)
...
def _require_secure_open_flags() -> None:
    if any(not hasattr(os, name) for name in _REQUIRED_FLAGS):
        raise OSError(errno.ENOTSUP, "Secure descriptor-relative traversal is unavailable")
...
def _open_directory_nofollow(path: Path) -> int:
    ...
    if not absolute.is_absolute():
        raise OSError(errno.EINVAL, "Directory path must resolve lexically to an absolute path")
