...
_CONCEPT_FIELDS = frozenset({"type", "title", "description", "tags", "confidence", "quotes"})
...
def _serialized(document: object) -> tuple[Any | None, tuple[_Issue, ...]]:
    if isinstance(document, bytes):
        ...
        try:
            ...
            return None, (_issue("/", "invalid_document", "Request must contain JSON values"),)
...
def _bounded_text(
    ...
    *,
    ...
    if len(canonical) > maximum:
        return None, [_issue(path, "too_long", f"Value must be at most {maximum} code points")]
    return canonical, []
...
def _validate_quotes(value: Any, path: str) -> tuple[tuple[_Quote, ...], list[_Issue]]:
    ...
    if len(value) > 8:
        issues.append(_issue(path, "too_many_items", "At most eight quotes are allowed"))
    parsed: list[_Quote] = []
    ...
    for index, raw in enumerate(value):
        ...
        if present == _LOCATOR_FIELDS:
            ...
            if type(raw_start) is int and type(raw_end) is int and not (0 <= raw_start < raw_end):
                issues.append(
                    _issue(
                        quote_path,
                        "invalid_range",
        ...
        if key in seen:
            issues.append(_issue(quote_path, "duplicate_item", "Quote objects must be unique"))
            continue
        seen.add(key)
        parsed.append(item)
...
def _parse_winddown(document: object) -> tuple[tuple[_Concept, ...], tuple[_Issue, ...]]:
    ...
    if len(raw_concepts) > 8:
        issues.append(_issue("/concepts", "too_many_items", "At most eight concepts are allowed"))
    ...
    for index, raw in enumerate(raw_concepts):
        ...
        if not isinstance(raw_kind, str):
            ...
            issues.append(_issue(f"{path}/type", "invalid_choice", "Unknown concept type"))
        else:
            kind = raw_kind
        ...
        if not isinstance(raw_tags, list):
            issues.append(_issue(f"{path}/tags", "invalid_type", "Tags must be an array"))
        else:
            if len(raw_tags) < 2:
                issues.append(
                    _issue(f"{path}/tags", "too_few_items", "At least two tags are required")
                )
            if len(raw_tags) > 5:
                issues.append(
                    _issue(f"{path}/tags", "too_many_items", "At most five tags are allowed")
                )
            valid_tags: list[str] = []
            ...
            for tag_index, tag in enumerate(raw_tags):
                ...
                if not isinstance(tag, str):
                    ...
                    issues.append(_issue(tag_path, "invalid_format", "Tag has an invalid format"))
                elif tag in seen_tags:
                    issues.append(_issue(tag_path, "duplicate_item", "Tags must be unique"))
                else:
                    seen_tags.add(tag)
        ...
        elif not math.isfinite(float(raw_confidence)) or not 0.5 <= raw_confidence <= 1.0:
            issues.append(
                _issue(
                    f"{path}/confidence",
                    "out_of_range",
        ...
        if canonical in seen_concepts:
            issues.append(_issue(path, "duplicate_concept", "Canonical concepts must be unique"))
            continue
...
def _parse_bind_document(document: object) -> tuple[tuple[_Quote, ...], tuple[_Issue, ...]]:
