...
def test_legitimate_parent_deletion_cascades_remove_root_event_and_fts(
    ...
    if parent == "assertion":
        production_store.conn.execute("DELETE FROM context_assertions WHERE id=?", (concept_id,))
    else:
        production_store.conn.execute("DELETE FROM context_evidence WHERE id=?", (evidence_id,))
...
def test_allocator_strictly_advances_past_observed_remote_integer_time(
    ...
    result = service.transition(concept_id, "retired", actor="owner", reason="strict advance")
...
def test_allocator_rejects_exhausted_observed_remote_time_and_rolls_back(
    ...
    with pytest.raises(RuntimeError, match="exhausted"):
        service.transition(concept_id, "retired", actor="owner", reason="must roll back")
    ...
    assert (
        production_store.conn.execute("SELECT count(*) FROM context_concept_events").fetchone()[0]
        == before_events
    )
...
def test_allocator_rejects_exhausted_local_origin_sequence_and_rolls_back(
    ...
    with pytest.raises(RuntimeError, match="exhausted"):
        service.transition(concept_id, "accepted", actor="owner", reason="must roll back")
