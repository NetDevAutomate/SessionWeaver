...
def test_winddown_binds_exact_quote_through_pinned_agent_context_contract(
    ...
    result = service.winddown("fixture-session-1", _document(_concept(quote)), actor="model-a")
...
def test_emoji_and_combining_character_offsets_are_unicode_code_points(
    ...
    result = service.winddown(
        ...
        _document(
            _concept(
                quote,
                quotes=[{"quote": quote, "evidence_id": evidence_id, "start": 1, "end": 4}],
            )
        ),
        actor="unicode-model",
    )
...
def test_quote_only_resolution_is_literal_unique_and_never_leaks_bodies(
    ...
    repeated = service.winddown("fixture-session-1", _document(_concept("echo")), actor="model")
    absent = service.winddown("fixture-session-1", _document(_concept("ECHO")), actor="model")
    ...
    assert all("secret-body" not in issue.message for issue in (*repeated.errors, *absent.errors))
...
def test_resolver_enforces_scope_session_lifecycle_and_evidence_withdrawal(
    ...
    if hidden_by == "scope":
        conn.execute(
            "INSERT INTO context_projects VALUES ('hidden-work','work','manual',?)", (_NOW,)
        )
        conn.execute(
        ...
        local = conn.execute("SELECT instance FROM context_access_state WHERE id=1").fetchone()[0]
        conn.execute(
...
def test_lifecycle_rules_and_retirement_leave_source_and_siblings_untouched(
    ...
    duplicate_accept = service.transition(first, "accepted", actor="owner", reason="repeat")
    ...
    assert (
        conn.execute("SELECT count(*) FROM sessions WHERE id='fixture-session-1'").fetchone()[0]
        == 1
    )
    assert (
        conn.execute(
...
def test_legacy_accept_is_refused_and_safe_bind_copies_metadata_without_auto_accept(
    ...
    refused = service.transition(legacy_id, "accepted", actor="owner", reason="cannot trust yet")
...
def test_injected_legacy_bind_failure_rolls_back_new_rows_old_retirement_and_clock(
    ...
    assert _state(production_store.conn) == baseline
    assert (
        _ConceptRepository(production_store.conn, now=lambda: _NOW).current_event(legacy_id)[
            "standing"
        ]
        == "proposed"
    )
