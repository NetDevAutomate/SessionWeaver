...
from session_weaver.ontology import (
    EXTRACTION_VERSION,
    ONTOLOGY_TABLES,
    CanonicalMessage,
    OntologyStatus,
...
def test_rebuild_materializes_tbox_structural_entities_and_shared_abox(
    ...
    _insert_session(conn, parent_id, project_path=project_path, git_branch="feat/ontology")
    _insert_session(
        conn,
        child_id,
        project_path=project_path,
        git_branch="feat/ontology",
        metadata=json.dumps({"transcript": f"/tmp/{parent_id}/subagents/agent-child.jsonl"}),
    )
    _insert_message(
        conn,
        "structural-parent",
...
def test_rebuild_and_reordered_source_insertion_have_the_same_logical_hash(
    ...
    first_structural = conn.execute("SELECT * FROM ontology_structural ORDER BY id").fetchall()
...
def test_incremental_reextracts_newer_and_missing_sessions_then_matches_full(
    ...
    conn = production_store.conn
    rebuild_ontology(conn)
    conn.execute("UPDATE ontology_build_state SET completed_at = '2026-09-07T13:00:00Z'")
...
def test_incremental_invalid_state_falls_back_to_full(
    ...
    if damage == "missing":
        ...
        conn.execute("UPDATE ontology_build_state SET extraction_version = 'tier1-obsolete'")
    else:
    ...
    assert conn.execute("SELECT extraction_version FROM ontology_build_state").fetchone() == (
        EXTRACTION_VERSION,
    )
...
def test_incremental_removes_absent_session_rows(
    ...
    assert conn.execute(
        "SELECT COUNT(*) FROM ontology_individual WHERE id = ?", (f"session:{session_id}",)
    ).fetchone() == (0,)
    assert conn.execute(
        """
...
def test_parent_metadata_without_a_live_parent_still_types_subagent(
    ...
    _insert_session(
        conn,
        "metadata-subagent",
        metadata=json.dumps({"transcript": f"/tmp/{missing_parent}/subagents/child.jsonl"}),
    )
    conn.commit()
