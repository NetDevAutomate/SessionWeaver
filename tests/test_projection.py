...
def test_project_writes_one_visible_bound_concept_without_mutating_database(
    ...
    assert report.unchanged == report.replaced == report.deleted == report.conflicts == 0
...
def test_project_authorizes_bound_and_provenanced_legacy_but_omits_unavailable_and_retired(
    ...
    assert service.transition(retired, "retired", actor="owner", reason="obsolete").writes == 1
    legacy = _seed_legacy(
        production_store,
    ...
    legacy_text = (tmp_path / "authorized-projection" / legacy_name).read_text(encoding="utf-8")
...
def test_project_rechecks_complete_bound_visibility_after_evidence_withdrawal(
    ...
    assert report.skipped_unavailable == 1
    assert (
        json.loads((tmp_path / "withdrawn-projection" / _MANIFEST).read_text(encoding="utf-8"))
        == {}
    )
...
def test_retirement_removes_only_the_exact_stale_managed_file(
    ...
    concept_ids = [
        row[0]
        for row in production_store.conn.execute("SELECT id FROM context_concepts ORDER BY title")
    ]
    out = tmp_path / "retirement-projection"
    first = service.project(out)
    initial_manifest = json.loads((out / _MANIFEST).read_text(encoding="utf-8"))
    stale_name = next(
        name for name, entry in initial_manifest.items() if entry["concept_id"] == concept_ids[1]
    )
    sibling_name = next(name for name in initial_manifest if name != stale_name)
    sibling_bytes = (out / sibling_name).read_bytes()

    assert (
        service.transition(concept_ids[1], "retired", actor="owner", reason="obsolete").writes == 1
    )
    second = service.project(out)
    ...
    assert list(json.loads((out / _MANIFEST).read_text(encoding="utf-8"))) == [sibling_name]
...
def test_filename_collision_is_refused_before_output_creation(
    ...
    monkeypatch.setattr("session_weaver.projection._filename", lambda _concept: "same.md")
...
def test_exact_crash_orphan_is_adopted_but_manifest_is_published(
    ...
    generated_name = next(name for name in _tree_bytes(complete) if name.endswith(".md"))
...
def test_manifest_failure_restores_prior_tree_and_cleans_invocation_artifacts(
    ...
    import session_weaver.projection as projection_module
    ...
    assert (
        service.transition(retire_id, "retired", actor="owner", reason="rollback fixture").writes
        == 1
    )
...
def test_access_generation_drift_before_or_after_publication_restores_prior_manifest(
    ...
    before = _tree_bytes(out)
    assert (
        service.transition(concept_id, "accepted", actor="owner", reason="force replacement").writes
        == 1
    )
    injected = False
    ...
    def change_generation(name: str) -> None:
        ...
        production_store.conn.execute("DELETE FROM context_projects WHERE id='drift-away'")
        production_store.conn.commit()
...
def test_stale_snapshot_rollback_preserves_user_modified_published_file(
    ...
    def modify_then_stale(name: str) -> None:
        ...
        production_store.conn.execute("DELETE FROM context_projects WHERE id='rollback-drift'")
        production_store.conn.commit()
...
def test_acceptance_replaces_same_filename_with_honest_standing(
    ...
    assert service.transition(concept_id, "accepted", actor="owner", reason="reviewed").writes == 1
...
def test_db_side_policy_digest_drift_raises_scope_error_and_rolls_back(
    ...
    def corrupt_policy_digest(name: str) -> None:
        ...
        try:
            raw.execute("UPDATE context_policy_state SET digest=? WHERE id=1", ("a" * 64,))
            raw.commit()
        finally:
            raw.close()
    ...
    raw = sqlite3.connect(production_store.db_path)
    try:
        raw.execute("UPDATE context_policy_state SET digest=? WHERE id=1", (original_digest,))
        raw.commit()
    finally:
        raw.close()
...
def test_partial_backup_cleanup_failure_is_recorded_and_never_touches_published_files(
    ...
    for concept_id in concept_ids:
        assert (
            service.transition(concept_id, "accepted", actor="owner", reason="force replace").writes
            == 1
        )
...
def test_rollback_preserves_managed_file_when_backup_is_tampered_at_commit_time(
    ...
    monkeypatch.setattr(projection_module, "_checkpoint", tamper_backup_then_fail_manifest)
