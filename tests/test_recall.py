...
def test_stop_set_is_pinned_verbatim_from_the_poc() -> None:
    assert STOP == frozenset(_PINNED_STOP_WORDS.split())
    assert len(STOP) == len(_PINNED_STOP_WORDS.split())
...
def test_authorized_concepts_rejects_a_project_mismatched_with_the_open_context(
    ...
    with open_context(production_store.db_path, project=None) as context:
        with pytest.raises(ValueError, match="project must match"):
            authorized_concepts(context, project="mismatched-project")


def test_fts_ranked_concept_ids_returns_empty_on_operationalerror() -> None:
    ...
    try:
        ...
        assert _fts_ranked_concept_ids(conn, '"unterminated') == []
...
def _validate(schema: dict[str, Any], instance: Any, defs: dict[str, Any] | None = None) -> None:
    ...
    if "$ref" in schema:
        ...
        _validate(defs[target.removeprefix("#/$defs/")], instance, defs)
        return
    ...
    if isinstance(instance, dict):
        ...
        for key, subschema in schema.get("properties", {}).items():
            if key in instance:
                _validate(subschema, instance[key], defs)
    if isinstance(instance, list):
        item_schema = schema.get("items")
        if item_schema is not None:
            for item in instance:
                _validate(item_schema, item, defs)
...
def test_recall_cli_json_output_matches_the_contract(
    ...
    assert (
        main(["recall", "clicontractterm", "--db", str(production_store.db_path), "--json"]) == 0
    )
