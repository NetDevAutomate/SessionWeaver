...
def _recent_visible_project_basenames(db: Path, *, limit: int) -> list[str]:
    """Distinct project basenames, newest-first, for scope-visible sessions.

    A directory/worktree basename is not guaranteed to appear verbatim in any
    message: the planner's tokenizer (ported unmodified from the PoC) keeps
    hyphens inside one token, and FTS5 splits on them when indexing content,
    so a heavily hyphenated basename (e.g. a git worktree name) can
    legitimately tokenize into a phrase nothing matches. The smoke test tries
    a window of the most recent basenames -- newest first -- and uses the
    first one that actually returns a hit, rather than assuming literally the
    single newest session's basename must match.
    """
...
def test_real_corpus_recall_smoke() -> None:
    ...
    try:
        ...
        basenames = _recent_visible_project_basenames(backup_path, limit=25)
        if not basenames:
            pytest.skip("no scope-visible session with a project path in the live backup")

        newest_report = None
        newest_seconds = 0.0
        for basename in basenames:
            started = perf_counter()
            candidate = recall(backup_path, basename)
            newest_seconds = perf_counter() - started
            if len(candidate.concepts) + len(candidate.sessions) >= 1:
                newest_report = candidate
                break
        if newest_report is None:
            pytest.skip(
                "none of the most recent scope-visible project basenames returned a hit"
            )
        assert len(newest_report.concepts) + len(newest_report.sessions) >= 1
