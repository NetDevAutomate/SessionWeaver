...
from session_weaver.winddown import _parse_bind_document, _parse_winddown
...
def test_exact_keys_are_required_for_concepts_and_quotes() -> None:
    ...
    assert ("/concepts/0/quotes/0/extra", "extra_field") in _codes(_document([quote_extra]))
...
def test_valid_boundaries_are_trimmed_and_tags_are_canonical() -> None:
        ...
        _document(
            [
                _concept(
                    ...
                    confidence=1,
                    quotes=[{"quote": "🙂e\u0301", "evidence_id": "ev", "start": 1, "end": 4}],
                )
            ]
        )
    )
    ...
    assert issues == ()
    assert parsed[0].title == "Twelve word title stays inside the exact word and character bounds"
...
def test_duplicate_tags_quotes_and_canonical_concepts_are_errors() -> None:
    ...
    assert ("/concepts/0/quotes/1", "duplicate_item") in _codes(_document([duplicate_quote]))
...
def test_one_and_eight_quotes_are_valid_and_nine_is_rejected() -> None:
    ...
    eight, eight_issues = _parse_winddown(
        _document([_concept(quotes=[{"quote": f"quote-{index}"} for index in range(8)])])
    )
...
def test_bind_document_accepts_only_one_to_eight_quote_locators() -> None:
    ...
    assert len(parsed) == 2
    assert issues == ()
    assert [(issue.path, issue.code) for issue in metadata_issues] == [("/title", "extra_field")]
    assert ("/quotes", "too_few_items") in {(issue.path, issue.code) for issue in empty_issues}
